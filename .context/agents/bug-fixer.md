...
Diagnose and fix bugs in the ai-coders-context CLI tool. This includes runtime errors in the interactive CLI flow, failures in LLM-backed fill/sync operations, frontmatter parsing issues, incorrect scaffold generation, workflow phase gate failures, and tree-sitter/LSP semantic analysis crashes.
...
- `src/services/fill/fillService.ts` -- FillService orchestrates LLM-based content generation; imported by 15+ files. Bugs here cascade widely.
...
- `src/services/ai/agents/documentationAgent.ts` -- Agentic LLM calls via Vercel AI SDK (`generateText`, `generateObject`). Provider config errors and token limit issues surface here.
...
4. **Check LLM provider config**: For AI-related failures, verify the provider factory chain: `src/services/llmClientFactory.ts` -> `src/services/ai/providerFactory.ts`. Ensure `LLMConfig.provider` matches one of `'openrouter' | 'openai' | 'anthropic' | 'google'` and the API key resolves via `getApiKeyFromEnv()`.
...
- **FillService glob patterns**: `src/services/shared/globPatterns.ts` defines default exclude patterns. Adding new file types to scaffolds without updating glob patterns causes them to be skipped during fill operations.
