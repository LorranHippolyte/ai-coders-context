...
**@ai-coders/context** (v0.7.1) is a CLI tool for generating codebase documentation and AI agent prompts. It scaffolds a `.context` directory structure containing docs, agents, plans, and skills, then optionally fills that scaffolding with AI-generated content using Anthropic, OpenAI, or Google AI providers. It also supports semantic code analysis via tree-sitter, a PREVC workflow engine, MCP server mode for AI tool integration, and context export/sync to Claude, Gemini, and Codex.

## Codebase Reference

The machine-readable codebase map lives at `.context/docs/codebase-map.json`. It contains file listings, dependency graphs, and structural metadata used by AI agents and the fill service.
...
- **MCP server**: accessible through the `serve` service for AI tool integration
...
- CLI commands exposed through commander (init, fill, sync, export, workflow, etc.)
- MCP server tools (via `src/services/mcp/`)
- Programmatic service APIs under `src/services/`
...
src/
  ...
  services/             # Core business logic
    ai/                 # AI provider abstraction (Anthropic, OpenAI, Google)
    autoFill/           # Auto-fill orchestration
    export/             # Context export to AI tools
    fill/               # Scaffolding fill service
    import/             # Context import
    init/               # .context scaffolding initializer
    mcp/                # MCP server mode
    plan/               # Plan management
    qa/                 # QA scaffolding service
    semantic/           # Tree-sitter semantic analysis
    sync/               # Context sync to AI tools (Claude, Gemini, Codex)
    workflow/           # PREVC workflow engine
    ... (and more)
...
6. Try the CLI: `npx ai-context init` to scaffold a `.context` directory in any project
